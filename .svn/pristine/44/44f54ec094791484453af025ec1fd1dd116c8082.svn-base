...
<font id="ordercount">当前订单(${total})</font>
<img src="./imgs/orgright.png" style="float:right; width: 6px;position: relative;top: 12px;" onclick="hereorderlist()">
<font style="float:right;color:#ff8854;font-size:10px;margin-right:5px;" onclick="hereorderlist()">前往历史订单</font>
</div>
...
<div style="width: 100%; height: 182px; background: #fff;" onclick="orderdetail(${p.order_id})">
	...
	<div style="height: 41px; line-height: 41px; float:right;">
		<span style="text-align: right; color:#857777;"><c:if test="${p.is_out == 0 }">堂食</c:if> <c:if test="${p.is_out == 1 }">外卖</c:if>：<c:if test="${p.state == 1 }"><font style="color: #ffba40;">店家未确认</font></c:if><c:if test="${p.state == 2 }"><font style="color: #57ceb0;">店家已确认 </font></c:if></span>
	</div>
	</div>
</div>
