...
public class OrderService extends OrderDetailService {
	...
	public float canDiscountOrderPrice(long order_id) {
		Double weighprice = (Double) dataDao.getFirstObjectViaParam(
				"select sum( quantity*price*weight ) from OrderDetail where order_id = :order_id and can_discount = 1 and is_weigh = :is_weigh and detail_flg >= 0 ", new String[] { "order_id",
						"is_weigh" }, order_id, 1);
		if (weighprice == null) {
			weighprice = 0d;
		}
		Double unweighprice = (Double) dataDao.getFirstObjectViaParam(
				"select sum( quantity*price ) from OrderDetail where order_id = :order_id and can_discount = 1 and is_weigh = :is_weigh and detail_flg >= 0 ", new String[] { "order_id", "is_weigh" },
				order_id, 0);
