...
<div class="statediv  <c:if test="${type==1 }">onchoose </c:if>" style="margin-left:0px;" onclick="allsalecount(1)">
日统计
</div>
<%-- <div class="statediv  <c:if test="${type==2 }">onchoose </c:if>" > --%>
<!-- 周统计 -->
<!-- </div> -->
<div class="statediv  <c:if test="${type==3 }">onchoose </c:if>" onclick="allsalecount(3)" >
月统计
</div>
