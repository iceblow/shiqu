...
public class UserService extends BaseService {
	...
	public HashMap<String, Object> thirdPartLogin(int type, String openid) throws CustomException {
		...
		if (userInfo != null) {
			...
			if (type == 1) {
				userInfo.setUser_name("QQ用户" + randomId);
				userInfo.setNick_name("QQ用户" + randomId);
				userInfo.setQq_openid(openid);
			} else {
				userInfo.setUser_name("微信用户" + randomId);
				userInfo.setNick_name("微信用户" + randomId);
	...
	public List<MyCoupon> getMyCoupon(long user_id) throws CustomException {
		...
		for (MyCoupon myCoupon : myCoupons) {
			...
			myCoupon.setStore_id(s.getStore_id());
		}
		return myCoupons;
	}
	...
	public HashMap<String, Object> getMyOrder(long user_id) throws CustomException {
		...
		Double total = (Double) dataDao.getFirstObjectViaParam("select sum(total_price + send_money) from Order where state > 3 and is_shop_order = 0 and  user_id = " + user_id, null);
		Double used = (Double) dataDao.getFirstObjectViaParam("select sum( price) from Order where state > 3 and is_shop_order = 0  and user_id = " + user_id, null);
	...
	public List<RechargeRecord> getRecord(long user_id, long card_id) {
		String hql = "from RechargeRecord where user_id = " + user_id + " and ( card_id = " + card_id + " or card_id = 0 ) order by charge_time desc";
	...
	public List<Store> getMsg(long user_id) {
		...
		String hqlmsg = "from MessageInfo where store_id = :sid  and find_in_set('" + user_id + "', user_ids ) > 0 order by addtime desc";
		List<Store> stores = (List<Store>) dataDao.getObjectsViaParam(hql, null);
		for (Store store : stores) {
			store.setPic_url(getFilePathById(store.getPic()));
