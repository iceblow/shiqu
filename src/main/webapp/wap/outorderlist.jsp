...
<div style="float:left; width: 50%; border-bottom: 1px solid #f9f9f9; font-size: 15px; text-align: center; line-height: 43px;" onclick="hereorderlist()">堂食</div>
<div style="float:left; width: 50%; border-bottom: 1px solid #ff8854; font-size: 15px; text-align: center; line-height: 43px;" onclick="outorderlist()">外卖</div>
</div>
...
<div style="width: 100%; height: 182px; background: #fff;" onclick="orderdetail(${p.order_id})">
	...
	<div style="height: 41px; line-height: 41px; float:right;">
		<span style="text-align: right; color:#857777;">外卖：<c:if test="${p.state == 3 }"><font style="color: #ff837a;">店家拒单</font></c:if><c:if test="${p.state == 4 }"><font style="color: #12c6d1;">已付款 </font></c:if><c:if test="${p.state == 5 }"><font style="color: #ccc7c7;">取消 </font></c:if></span>
	</div>
	</div>
</div>
