...
<script type="text/javascript">
	...
	function findshoplist(){
		...
		window.location.href = "./agentlist?keywords="+keywords;
	}
	...
	function clearinput(){
		...
		$("#alert_name").val("");
		$("#password").val("");
		$("#myModalLabel").text("添加代理商用户");
	}
	
	function relative(id){
		window.location.href = "./relativeAgentStoreList?id="+id;
	}
	
	function myrelative(id){
		window.location.href = "./myrelativeAgentStore?id="+id;
	}
</script>
</head>
<body>
					...
					<div class="row-fluid">
						<div class="span12">
							...
							<table class="table table-condensed table-hover table-bordered">
								...
								<tbody>
									<c:forEach items="${agentlist}" var="s">
										<tr>
											<td>${s.name}</td>
											<td>
												<button class="btn btn-small btn-info" type="button" onclick="edit(${s.id},'${s.name }')" style="float: left;">代理商管理</button>
							...
							<c:if test="${pageCount>1}">
                                ...
                                var a = [];
                                  ...
                                  if (pageindex == 1) {
                                    ...
                                    a[a.length] = "<li><a onclick=\"previousPage("+pageindex+",'./agentlist?keywords="+keywords+
                                    		"&')\" class=\"page_prev\">&laquo;</a></li>";
                                  }
                                  function setPageList() {
                                    if (pageindex == i) {
                                      a[a.length] = "<li class='active'><a onclick=\"goPage('./agentlist?keywords="+keywords+
                              		"&',"+i+")\" class=\"on\">" + i + "</a></li>";
                                    } else {
                                      a[a.length] = "<li><a onclick=\"goPage('./agentlist?keywords="+keywords+
                              		"&',"+i+")\">" + i + "</a></li>";
                                    }
                                  }
                                  //总页数小于10
                                  if (count <= 10) {
                                    ...
                                    if (pageindex <= 4) {
                                      ...
                                      a[a.length] = "...<li><a onclick=\"goPage('./agentlist?keywords="+keywords+"&',"+count+")\">" + count + "</a></li>";
                                    } else if (pageindex >= count - 3) {
                                      a[a.length] = "<li><a onclick=\"goPage('./agentlist?keywords="+keywords+"&',1)\">1</a></li>...";
                                  ...
                                  if (pageindex == count) {
                                        ...
                                        "<li><a style='border-left-width: 1px; border-radius: 4px;' class=\"jump_btn\" onclick=\"gotoPage('./agentlist?keywords="+keywords+"&',"+$("#pageSize").val()+")\")\">确定</li>";
                                      } else {
                                        a[a.length] = 
                                            "<li><a style='border-bottom-right-radius:4px;border-top-right-radius:4px; ' onclick=\"nextPage("+$("#pageIndex").val()+",'./agentlist?keywords="+keywords+"&')\" "+
