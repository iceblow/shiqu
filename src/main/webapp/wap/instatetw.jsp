...
$(function(){
	...
	html = '';
	if(hascard != 0){
		html += '<option value="4">充值卡</option>';
...
function payorder(order_id,coupon_id){
	...
	if(selecttype == 4){
		window.location.href = "./paycardorder?order_id="+order_id+"&coupon_id="+coupon_id+"&pay_type="+selecttype;
	}else{
		callpay();
	}
}
...
<div style="width: 92%; height: 100%; margin: 0 auto;">
	...
	<input value="${hascard }" type="hidden" id="hascard"/>
	<select style="float:right	; color: #ff4532; margin-right: 10px; -webkit-appearance:none; margin-top: 16px; border: 0px;  text-align: right;" id="selecttype">
...
<div style="width: 92%; height: 60px; margin: 0 auto; line-height: 60px;">
	...
	<div style="float:right; margin-top: 13px; background: #ff8854; color: #fff;" class="paybutton" onclick="payorder(${order.order_id},${mycoupon.id })">付款</div>
	</c:if>
</div>
</div>
...
<input type="hidden" value="${mycoupon.id }" id="coupon_id">
