...
public class ShopService extends OrderDetailService {
	...
	public List<MenuCategory> getStoreMenu(long sid) {
		...
		for (MenuCategory menuCategory : menuCategories) {
			menuCategory.setMenu_list(
					(List<Menu>) dataDao.getObjectsViaParam("from Menu where delflag = 0 and sell_out = 0 and store_id = " + sid + " and category  = '" + menuCategory.getName() + "'", null));
		}
	 ...
	 */
	@Transactional(readOnly = false, propagation = Propagation.REQUIRED)
	public void changeTable(long from_id, long to_id, long user_id) throws CustomException {
		...
		changeTablePrint(from_id, to_id, user_id);
	}
	...
	public long addOrder(String user_id, String store_id, String table_id, String comment, String menu_info, String total_price, String price, int people_num) throws CustomException {
		long order_id = 0l;
	...
	@Transactional(readOnly = false, propagation = Propagation.REQUIRED)
	public void cancelPresent(long order_id, long order_menu_id) throws CustomException {
		OrderDetail orderDetail = dataDao.getObjectById(OrderDetail.class, order_menu_id);
		if (orderDetail == null) {
			throw new CustomException(Constants.WEIGHT_EXCEPTION);
		}
		...
		if (orderDetail.getIs_free() != 1) {
			throw new CustomException(Constants.CANCEL_PRESENT_EXCEPTION);
		}
		Float originPrice = (Float) dataDao.getFirstObjectViaParam("select price from Menu where menu_id = " + orderDetail.getMenu_id(), null);
		if (originPrice == null) {
			originPrice = 0.0f;
		}
		orderDetail.setPrice(originPrice);
		if (orderDetail.getIs_weigh() == 0) {
			order.setTotal_price(CommonUtils.twoDecimalP(order.getTotal_price() + originPrice * orderDetail.getQuantity()));
			order.setPrice(CommonUtils.twoDecimalP(order.getPrice() + originPrice * orderDetail.getQuantity()));
		} else {
			order.setTotal_price(CommonUtils.twoDecimalP(order.getTotal_price() + originPrice * orderDetail.getQuantity() * orderDetail.getWeight()));
			order.setPrice(CommonUtils.twoDecimalP(order.getPrice() + originPrice * orderDetail.getQuantity() * orderDetail.getWeight()));
		}
