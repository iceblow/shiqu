		...
		$.ajax({
			...
			success:function(msg){
				...
				if(code==1){
					...
					var order_id = msgdata.order_id;
					var newprice = msgdata.newprice;
					$("#allprice_"+order_id).text('￥'+newprice);
					alert("称重成功!");
					hideWeightAlert();
				}else{
					alert(msgdata.message);
...
<div id="content">
	...
	<tr class="tablehead">
	<th style="width:6.25%;">流水号</th> 	
	<th style="width:6.25%;">下单人</th> 
	<th style="width:6.25%;">手机号</th> 
	<th style="width:6.25%;">订单来源</th> 
	<th style="width:6.25%;">就餐方式</th>
	<th style="width:6.25%;">地址</th> 
	<th style="width:6.25%;">状态</th> 
	<th style="width:6.25%;">菜品总价</th> 
	<th style="width:6.25%;">支付金额</th> 
	<th style="width:6.25%;">使用优惠</th> 
	<th style="width:6.25%;">桌台</th> 
	<th style="width:6.25%;">备注</th> 
	<th style="width:6.25%;">下单时间</th>
<!-- 	<th style="width:7%;">确认时间</th>    -->
	<th style="width:6.25%;">支付时间</th> 	
	<th style="width:6.25%;">支付方式</th> 
	<th style="width:6.25%;">操作</th> 	 
	</tr>
	<c:forEach items="${orderlist}" var="p">
	<tr>
	<th style="width:6.25%;" class="border_right tablethcss">${p.serial_num }</th> 	
	<th style="width:6.25%;" class="border_right tablethcss">${p.order_name }</th> 
	<th style="width:6.25%;" class="border_right tablethcss">${p.phone }</th> 
	<th style="width:6.25%;" class="border_right tablethcss"><c:if test="${p.is_shop_order==0 }">C端</c:if><c:if test="${p.is_shop_order==1 }">B端</c:if></th> 
	<th style="width:6.25%;" class="border_right tablethcss"><c:if test="${p.is_out==0 }">堂食</c:if><c:if test="${p.is_out==1 }">外卖</c:if></th> 
	<th style="width:6.25%;" class="border_right tablethcss">${p.address }</th> 
	<th style="width:6.25%;" class="border_right tablethcss"><c:if test="${p.state==1 }">未确认</c:if>
	<c:if test="${p.state==2 }">已确认</c:if>
	<c:if test="${p.state==3 }">已拒单</c:if>
	<c:if test="${p.state==4 }">已完成</c:if>
	<c:if test="${p.state==5 }">已取消 </c:if>
	</th> 
	<th style="width:6.25%;" class="border_right tablethcss"><fmt:formatNumber value="${p.total_price }" pattern="##.##" minFractionDigits="2" ></fmt:formatNumber></th> 
	<th style="width:6.25%;" class="border_right tablethcss"><fmt:formatNumber value="${p.price }" pattern="##.##" minFractionDigits="2" ></fmt:formatNumber> </th> 
	<th style="width:6.25%;" class="border_right tablethcss"><fmt:formatNumber value="${p.total_price+p.send_money-p.price }" pattern="##.##" minFractionDigits="2" ></fmt:formatNumber>  </th> 
	<th style="width:6.25%;" class="border_right tablethcss">${p.table_no }</th> 
	<th style="width:6.25%;" class="border_right tablethcss">${p.comment }</th> 
	<th style="width:6.25%;" class="border_right tablethcss"><fmt:formatDate value="${p.order_time }" pattern="yyyy-MM-dd HH:mm:ss"/></th>
<%-- 	<th style="width:7%;" class="border_right tablethcss"><fmt:formatDate value="${p.dining_time }" pattern="yyyy-MM-dd HH:mm:ss"/></th>    --%>
	<th style="width:6.25%;" class="border_right tablethcss"><fmt:formatDate value="${p.check_time }" pattern="yyyy-MM-dd HH:mm:ss"/></th> 	
	<th style="width:6.25%;" class="border_right tablethcss"><c:if test="${p.state==4 }"><c:if test="${p.pay_type==1 }">现金</c:if><c:if test="${p.pay_type==2 }">支付宝</c:if><c:if test="${p.pay_type==3 }">刷卡</c:if><c:if test="${p.pay_type==4 }">充值卡</c:if><c:if test="${p.pay_type==5 }">签单</c:if><c:if test="${p.pay_type==6 }">免单</c:if></c:if></th>  
	<th style="width:6.25%; min-width: 100px;" class="tablethcss">
	...
	<td colspan="16" style="height:86px;">
...
<div style="background-color:#f5f6fa;width:100%;height:30px;line-height:20px;padding-left:20px;font-size:12px;font-family:'宋体';"><c:if test="${p.is_out ==1 }">外卖地址:${p.address }</c:if></div>
...
<div class="alert_content_div_nobold" style="width:75px;" id="weight_${q.id }">
...
<c:if test="${p.is_out==1 }">
<div style="margin-left:20px;margin-right:20px;height:49px;line-height:49px;">
<div class="alert_content_div_nobold" style="width:200px;">
总计:<font id="allprice_${p.order_id }"><fmt:formatNumber value="${p.price }" type="currency"/></font>
</div>
<c:if test="${p.state==1 }">
<div class="table_button_text" style="float: right;color: #ffffff;margin-right: 2%;cursor: pointer;height: 40px;width: 100px;line-height: 40px; background-color: #6097fb; margin-top: 5px;cursor:pointer" onclick="refuseOrder(${p.order_id})">拒接</div>
<div class="table_button_text" style="float: right;color: #ffffff;margin-right: 2%;cursor: pointer;height: 40px;width: 100px;line-height: 40px; background-color: #6097fb; margin-top: 5px;cursor:pointer" onclick="sureOrder(${p.order_id})">确认</div>
</c:if>
</div>
</c:if>
</div>
