...
<div class="store_list">
	...
	<div class="topdiv" onclick="turnstore(${store_id})">店铺</div>
	</div>
	...
	<div style="width:100%;  float:left;" id="content_form">
		...
		<input type="hidden" name="menuids" value="${q.menu_id }">
		<input type="hidden" id="num_${q.menu_id }" name="quantitys" value="0">
...
<c:forEach items="${categorylist}" var="p" >
		<c:forEach items="${p.menu_list}" var="q"  varStatus="status">
		<img src="./imgs/close.png" style="z-index:102;position:fixed;width:32px;top:34px;display:none;" class="closebtn" id="close_${q.menu_id }" onclick="hidealert(${q.menu_id})">
	<div class="alert" id="alert_${q.menu_id }">
		<div style="border-radius:8px 8px 0px 0px" class="alertimgdiv">
		<img src="${q.pic_url }"  style="border-radius:8px 8px 0px 0px;"  class="alertimg" >
		</div>
		...
		<div class="alertdescript" style="line-height:16px;display: -webkit-box;-webkit-box-orient: vertical;-webkit-line-clamp: 3;text-overflow:ellipsis;  overflow:hidden;height:48px; clear: both;padding-left:9px;padding-right:9px;font-size:12px;color:#9f9f9f;margin-bottom:20px;">
		${q.description }
		</div>
	</div>
		</c:forEach>
...
// 	$(".alert").css("top",top);
	...
	$(".alertimgdiv").css("width",width);
	$(".alertimgdiv").css("height",width);
	$(".alertimg").css("width",width);
	$(".alertimg").css("height",width);
