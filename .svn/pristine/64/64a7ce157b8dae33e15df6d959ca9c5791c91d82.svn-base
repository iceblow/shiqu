...
public class MaShopService extends BaseService {
	...
	public List<StoreCategory> getCategory() {
		...
		if(list != null && list.size() > 0){
			for (StoreCategory storeCategory : list) {
				if(storeCategory.getPic_id() != 0){
					storeCategory.setPic_url(getFilePathById(storeCategory.getPic_id()));
				}
			}
		}
		return list;
	...
	public void editCategory(Long cid, String name, Long pic_id) {
		StoreCategory storeCategory = dataDao.getObjectById(StoreCategory.class, cid);
		if (storeCategory != null) {
			if(pic_id != null){
				storeCategory.setPic_id(pic_id);
			}
	...
	@Transactional(readOnly = false, propagation = Propagation.REQUIRED)
	public void addCategory(String name, long pic_id) {
	...
	public StoreCategory getStoreCategoryById(Long category_id) {
		StoreCategory category = dataDao.getObjectById(StoreCategory.class, category_id);
		return category;
	}

	public List<MenuUnit> getMenuUnitList() {
		List<MenuUnit> list = dataDao.getAllObject(MenuUnit.class);
		return list;
	}

	public MenuUnit getMenuUnit(Long unit_id) {
		MenuUnit unit = dataDao.getObjectById(MenuUnit.class, unit_id);
		return unit;
	}

	@Transactional(readOnly = false, propagation = Propagation.REQUIRED)
	public void updateMenuUnit(MenuUnit unit) {
		dataDao.updateObject(unit);
	}

	@Transactional(readOnly = false, propagation = Propagation.REQUIRED)
	public void addMenuUnit(MenuUnit unit) {
		dataDao.addObject(unit);
	}

	@Transactional(readOnly = false, propagation = Propagation.REQUIRED)
	public void delMenuUnit(MenuUnit unit) {
		dataDao.deleteObject(unit);
	}
}
