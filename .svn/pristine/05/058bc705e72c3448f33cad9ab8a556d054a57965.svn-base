...
public class ManageCommonController extends ManageBaseController {
	...
	public ModelAndView refuselist(){
		...
		mov.setViewName("manage/refuselist");
		return mov;
	}
	
	@RequestMapping("/editrefuse")
	public ModelAndView editrefuse(String content, Long id){
		ModelAndView mov = new ModelAndView();
		if(id != 0){
			Refuse refuse = maCommonService.getRefuse(id);
			refuse.setContent(content);
			maCommonService.updateRefuse(refuse);
		}else{
			Refuse refuse = new Refuse();
			refuse.setContent(content);
			maCommonService.addRefuse(refuse);
		}
		mov.setViewName("redirect:/manage/refuselist");
		return mov;
	}
	
	@RequestMapping("/delrefuse")
	public ModelAndView delrefuse(Long id){
		ModelAndView mov = new ModelAndView();
		Refuse refuse = maCommonService.getRefuse(id);
		if(refuse != null){
			maCommonService.delRefuse(refuse);
		}
		mov.setViewName("redirect:/manage/refuselist");
		return mov;
	}
