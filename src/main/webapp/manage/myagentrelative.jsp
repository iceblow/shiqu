	...
	function relative(id){
		...
		if(confirm("是否取消关注？")){
			$.ajax({
				type: "post",
				url: "./cancelrelativeagentStore",
				data: {store_id :id, agentid :agentid},
				success: function(data){
					if(data == "success"){
						alert("关联成功!");
						window.location.reload();
				...
				<div class="span10">
					...
					<div class="row-fluid">
						<div class="span12">
							<div class="row-fluid">
								<div class="span6">
									<button class="btn btn-small btn-info" type="button" onclick="relativemany()" style="float: left;margin-left: 5px;">取消关联商店</button>
								</div>
								<div class="span2">
								</div>
							</div>
								...
								<tbody>
									<c:forEach items="${storelist}" var="s">
										<tr>
											...
											<td>${s.phone}</td>
											<td>
												<button class="btn btn-small btn-info" type="button" onclick="relative(${s.store_id})" style="float: left;margin-left: 5px;">取消关联商店</button>
											</td>
										</tr>
									</c:forEach>
								</tbody>
							...
							<c:if test="${pageCount>1}">
                                ...
                                var a = [];
                                  ...
                                  if (pageindex == 1) {
                                    ...
                                    a[a.length] = "<li><a onclick=\"previousPage("+pageindex+",'./myrelativeAgentStore?')\" class=\"page_prev\">&laquo;</a></li>";
                                  }
                                  function setPageList() {
                                    if (pageindex == i) {
                                      a[a.length] = "<li class='active'><a onclick=\"goPage('./myrelativeAgentStore?',"+i+")\" class=\"on\">" + i + "</a></li>";
                                    } else {
                                      a[a.length] = "<li><a onclick=\"goPage('./myrelativeAgentStore?',"+i+")\">" + i + "</a></li>";
                                    }
                                  }
                                  //总页数小于10
                                  if (count <= 10) {
                                    ...
                                    if (pageindex <= 4) {
                                      ...
                                      a[a.length] = "...<li><a onclick=\"goPage('./myrelativeAgentStore?',"+count+")\">" + count + "</a></li>";
                                    } else if (pageindex >= count - 3) {
                                      a[a.length] = "<li><a onclick=\"goPage('./myrelativeAgentStore?',1)\">1</a></li>...";
                                  ...
                                  if (pageindex == count) {
                                        ...
                                        "<li><a style='border-left-width: 1px; border-radius: 4px;' class=\"jump_btn\" onclick=\"gotoPage('./myrelativeAgentStore?',"+$("#pageSize").val()+")\")\">确定</li>";
                                      } else {
                                        a[a.length] = 
                                            "<li><a style='border-bottom-right-radius:4px;border-top-right-radius:4px; ' onclick=\"nextPage("+$("#pageIndex").val()+",'./myrelativeAgentStore?')\" "+
