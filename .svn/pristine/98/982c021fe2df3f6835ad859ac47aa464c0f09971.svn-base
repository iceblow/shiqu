...
public class ApiShopController extends ApiBaseController {
	...
	public String changeTable(HttpServletRequest request, HttpServletResponse rep) {
		...
		try {
			...
			shopService.changeTable(CommonUtils.parseLong(from_id, 0), CommonUtils.parseLong(to_id, 0),CommonUtils.parseLong(user_id, 0));
			map.putAll(Constants.SUCCESS_RESULT_MAP);
			return gson.toJson(map);
			// return mapper.writeValueAsString(map);
		} catch (CustomException e) {
	...
	public String addOrder(HttpServletRequest request, HttpServletResponse rep) {
		...
		try {
			...
			long oid = shopService.addOrder(user_id, store_id, table_id, comment, menu_info, total_price, price, CommonUtils.parseInt(people_num, 1));
			if (oid > 0) {
				shopService.sendOrderPrint(oid, 1);
			}
			map.putAll(Constants.SUCCESS_RESULT_MAP);
			return gson.toJson(map);
			// return mapper.writeValueAsString(map);
	...
	public String cashier(HttpServletRequest request, HttpServletResponse rep) {
		...
		try {
			...
			shopService.cashier(CommonUtils.parseLong(order_id, 0), CommonUtils.parseLong(coupon_id, 0), CommonUtils.parseFloat(discount, 0.0f), CommonUtils.parseFloat(moling, 0.0f, 2), type,
	...
	public String checkCoupon(HttpServletRequest request, HttpServletResponse rep) {
		...
		try {
			...
			CommonUtils.validateEmpty(total_price);
			CommonUtils.validateEmpty(store_id);
			MyCoupon mycoupon = shopService.checkCoupon(coupon_no, total_price, CommonUtils.parseLong(store_id, 0));
			map.put("coupon", mycoupon.getCoupon());
			map.put("mycoupon_id", mycoupon.getId());
			map.putAll(Constants.SUCCESS_RESULT_MAP);
			return gson.toJson(map);
			// return mapper.writeValueAsString(map);
		} catch (CustomException e) {
	 ...
	 */
	@ResponseBody
	@RequestMapping(value = "/cancelpresent", produces = "application/json; charset=UTF-8")
	public String cancelpresent(HttpServletRequest request, HttpServletResponse rep) {
		HashMap<String, Object> map = new HashMap<String, Object>();
		try {
			request.setCharacterEncoding("utf-8");
			String order_id = request.getParameter("order_id");
			...
			CommonUtils.validateEmpty(order_id);
			CommonUtils.validateEmpty(ordered_menu_id);
			shopService.cancelPresent(CommonUtils.parseLong(order_id, 0), CommonUtils.parseLong(ordered_menu_id, 0));
			map.putAll(Constants.SUCCESS_RESULT_MAP);
			return gson.toJson(map);
			// return mapper.writeValueAsString(map);
		} catch (CustomException e) {
